...
export default class Keycloak {
    ...
    async #fetchAccessToken(url, code, clientId, redirectUri, pkceCodeVerifier) {
        ...
        if (this.#dpopProvider) {
            return await this.#fetchDPoPAccessToken(url, body)
        } else {
    ...
    /**
     * Fetch the access token with DPoP proof attached.
     * Handles nonce requirements per RFC 9449 Section 8.
     * @param {string} url
     * @param {URLSearchParams} body
     * @returns {Promise<AccessTokenResponse>}
     */
    async #fetchDPoPAccessToken(url, body) {
        const dpopProvider = this.#dpopProvider
        if (!dpopProvider) {
            ...
            if (ex instanceof NetworkError && ex.response.status === 400) {
                ...
                if (dpopNonce) {
                    ...
                    if (errorBody.error === 'use_dpop_nonce') {
                        // Store the nonce and retry with it
                        await dpopProvider.updateAuthServerNonce(dpopNonce)
                        const proof = await dpopProvider.generateDPoPProof(url, 'POST', undefined, dpopNonce)
                        const headers = new Headers()
                        headers.set('DPoP', proof)
                        headers.set('Accept', CONTENT_TYPE_JSON)
                        const retryResp = await fetchWithErrorHandling(url, {
                            method: 'POST',
                            credentials: 'include',
                            body,
                            headers
                        })
                        // RFC 9449 Section 8.2: capture nonce from retry response as well
                        const retryNonce = retryResp.headers.get('DPoP-Nonce')
                        if (retryNonce) {
                            await dpopProvider.updateAuthServerNonce(retryNonce)
                        }
                        const tokenResponse = await retryResp.json()
                        return this.#validateDPoPTokenType(tokenResponse)
                    }
                }
            }
            // Re-throw if not a use_dpop_nonce error
            throw ex
        }
    }
    ...
    async #fetchRefreshToken(url, refreshToken, clientId) {
        ...
        if (this.#dpopProvider) {
            return await this.#fetchDPoPRefreshToken(url, body)
        } else {
            return await fetchJSON(url, {
                method: 'POST',
                credentials: 'include',
    ...
    /**
     * Fetch the refresh token with DPoP proof attached.
     * Handles nonce requirements per RFC 9449 Section 8.
     * @param {string} url
     * @param {URLSearchParams} body
     * @returns {Promise<AccessTokenResponse>}
     */
    async #fetchDPoPRefreshToken(url, body) {
        const dpopProvider = this.#dpopProvider
        if (!dpopProvider) {
            throw new Error('DPoP provider not initialized')
        }
